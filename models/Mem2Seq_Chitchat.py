...
class Mem2SeqChitChat(nn.Module):
    ...
    def evaluate_batch(self, batch_size, input_batches, input_lengths, target_batches, target_lengths, target_index,
        ...
        for t in range(self.max_r):
            ...
            top_ptr_i = torch.gather(input_batches[:, :, 0], 0, Variable(toppi.view(1, -1)))
            next_in = [top_ptr_i.squeeze(0)[i].data.item() if (int(toppi.squeeze(0)[i].item()) < input_lengths[i] - 1) else
                       topvi.squeeze(0)[i].item() for i in range(batch_size)]
    ...
    def save_decode_results(self, query, ref, hyp, bleu_score):
        # name_data = "KVR/" if self.task == '' else "BABI/"
        name_data = 'ChitChat/'

        dec_type = str(self.name) + str(bleu_score)
        directory = 'save/decodes/mem2seq-' + name_data
        file_name = str(self.task) + 'HDD' + str(self.hidden_size) + 'BSZ' + str(
            args['batch']) + 'DR' + str(self.dropout) + 'L' + str(self.n_layers) + 'lr' + str(self.lr) + str(dec_type) + \
                    ".decodes"
        ...
        with codecs.open(os.path.join(directory, file_name), 'w', encoding='utf-8') as f:
            for i in range(len(ref)):
                f.write("query : " + query[i] + '\n')
                f.write("ref : " + ref[i] + '\n')
                f.write("pred : " + hyp[i] + '\n')
                # extra empty line
                f.write("\n")
